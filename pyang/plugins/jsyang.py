"""JS-Tree and yang output plugin
...
class JSTreePlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--jsyang-no-path",
                                 dest="jsyang_no_path",
                                 action="store_true",
                                 help="""Do not include paths to make
                                       page less wide"""),
            optparse.make_option("--jsyang-path",
                                 dest="jsyang_path",
                                 help="Subtree to print"),
            optparse.make_option("--jsyang-prefix",
                                 dest="jsyang_prefix",
                                 help="prefix of path"),
            ]

        g = optparser.add_option_group("JSYang output specific options")
...
def emit_header(modules, fd, ctx):
    title = "check yang model by tree"
...
def emit_tree(modules, fd, ctx, path):
    ...
    for module in modules:
        ...
        objectID = str(module.pos.line) + '$' + module.pos.ref
        print("Module Position: %s" % objectID)
        levelcnt[1] += 1
